...
{
	//Здесь присваивается индекс потока внутри блока переменной id. Каждый поток будет обрабатывать один элемент массива.
	int id = threadIdx.x;
	if(id < size && id >= 0) 
...
public:
	...
	void forward(std::string pathToFile, bool print){
		...
		NN layer1("weight1.bin", "biases1.bin", 1024, 256, true);
		array = layer1.Linear(array);
		NN layer2("weight2.bin", "biases2.bin", 256, 16, true);
		array = layer2.Linear(array);
		NN layer3("weight3.bin", "biases3.bin", 16, 1, true);
...
int main()
{
	...
	Net model;
	model.forward("inputs.bin", true);
	auto elapsed = std::chrono::high_resolution_clock::now() - start;
        long long seconds = std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count();
		
        std::cout << "Время работы: "<< seconds << " ms"<< std::endl;
